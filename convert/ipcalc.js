function h_initArray() {
    ...
    for (var i = 0; i < this.length; i++) this[i] = h_initArray.arguments[i];
}
    ...
    if (f.nwclass.value != "Illegal") {
        ...
        if (f.subsuper.value == "子网是") {
            ...
            if (f.nwclass1.value == "C类网") {
                ...
                str += "<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" class=\"table table-bordered\">";
...
function compute2(f) {
    var cf = f.cf[1].value;
    if (code == "MSIE") {
        var node = f.node[1].value;
        var nw = f.network[1].value;
    } else {
        var node = f.node[1].value;
        var nw = f.network[1].value;
    }
